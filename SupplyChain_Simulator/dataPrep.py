...
def dataPrep(H):
    ...
    for line in ChainFile:
        ...
        if attList[4] == -1:
            ...
            KPur = dict(zip([-1], [.001] )) 
            # No Children so each part worths 1
            Value = 1
            # Margin is zero, no mark up
            Margin = 0
            #------------------End of Parameters------------------#
            # Construct Supplier...
            SupplierDict[attList[0]] = Supplier(attList[0], attList[1], attList[2],
                                                ...
                                                -1, -1, 0, list(), list(),
                                                thetas, KI, ΚΟ, KPro, KPur, Value, Margin)
        # Case: Rest of suppliers
            ...
            # We will want to update this, but only once and layer by layer
            Value = 0
            # Margin is zero, no mark up
            Margin = 0
            #------------------End of Parameters------------------#
            ...
            SupplierDict[attList[0]] = Supplier(attList[0], attList[1], attList[2], 
                                                ...
                                                spec6, -1, 0, list(), list(),
                                                thetas, KI, KO, KPro, KPur, Value, Margin)
    ...
    for depth in [3,2,1,0]:
        for ID, value in SupplierDict.items():
            if SupplierDict[ID].treeDepth == depth and depth != 0:
                # Add margin for a real per product value
                SupplierDict[ID].Value += SupplierDict[ID].Margin
                # The value per product of parent's is a combination of its sons'
                size = SupplierDict[SupplierDict[ID].ParentLabel].ProductDemands
                SupplierDict[SupplierDict[ID].ParentLabel].Value += int(SupplierDict[ID].Value)*int(size[ID])
                VR.write(' '.join([str(int(SupplierDict[ID].Label)), \
                                   str(SupplierDict[ID].ParentLabel), \
                                   str(SupplierDict[ID].treeDepth), \
                                   str(SupplierDict[ID].Value), '\n']))
            if SupplierDict[ID].treeDepth == depth and depth == 0:
                # Add margin for a real per product value but no parent (-1)
                SupplierDict[ID].Value += SupplierDict[ID].Margin
                VR.write(' '.join([str(int(SupplierDict[ID].Label)), \
                                   str(SupplierDict[ID].ParentLabel), \
                                   str(SupplierDict[ID].treeDepth), \
